...
// depending on error render corresponding error box at top of screen
...
app.getPhotos = (searchTerm) => {
  ...
  fetch(unsplashEndpoint)
    ...
    .then((jsonData) => {
      if (jsonData.total === 0) {
        throw new Error(
          '<i class="fas fa-exclamation-circle"></i> No results found.  Please try again.'
        );
      }
...
// Check for photographer twitter and render if it exists otherwise dont render anything
...
// Check for photographer instagram and render if it exists otherwise dont render anything
